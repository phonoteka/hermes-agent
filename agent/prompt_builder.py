...
KANBAN_GUIDANCE = (
    ...
    "reviewer can approve+unblock or request changes. Proof-loop phase "
    "cards are the exception to this exception: when the task body or loaded "
    "skills mention `repo-task-proof-loop-hermes`, `PARENT_REVIEW_*`, "
    "`READY_FOR_PARENT_REVIEW`, or an explicit generated review gate, "
    "normal review handoff MUST use `kanban_complete` with a "
    "`READY_FOR_PARENT_REVIEW`/`READY_FOR_VERIFY_*` summary. Use "
    "`kanban_block` there only when required artifacts/evidence cannot be "
    "produced locally or a true external decision is needed.\n"
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        try:
            host_lines.append(f"Current working directory: {get_runtime_cwd(os.getcwd())}")
        except OSError:
            pass
